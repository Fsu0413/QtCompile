...
参考 {{% QtBug 96339 %}} - ~~暂时~~使用 CMake 3.20.2 绕过这个问题

## QtOpcUa 的问题（{{% date 2024 4 19 %}}更新）
...
{{% date 2024 4 19 %}}更新：
Qt 6.2.8 发布，但是 open62541 还是 1.1 版本。  
懒得再看了，6.2 系列不再附带 QtOpcUa。
...
{{%date 2024 5 28 %}}更新：   
Qt 6.5.3 也加入放弃 QtWebEngine 豪华午餐。    
目前只有 macOS 是这种情况，Windows 什么时候碰到问题了什么时候再删。
...
由于此前的调查总是碰壁，如果以后出了补丁还是无法应用的话，我将（大概率）不会调查解决方案，而只是等待可适用该补丁的版本再适用。
