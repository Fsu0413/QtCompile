...
## `xz` 后门 （{{% date 2024 4 6 %}}更新，下次更新时将去除这段内容）
...
但是这个家伙的代码同时存在于 `libarchive`（在 macOS 和 Windows 上存在。`bsdtar` 来源于这个库）中。`libarchive` 目前已经（{{% date 2024 4 6 %}}更新）~~开始重新评审~~重新评审完这个家伙所做的代码了，并且已经修改了部分内容。

这一部分内容会在事情明朗之前一直放在这里，我将持续关注相关内容。~~如果有必要的话，我可能会对所有这个版本库中的内容做重新打包。~~（{{% date 2024 4 6 %}}更新）看来没必要，现在所有的分析结果都没有提到后门对正常的压缩 / 解压缩的代码的影响。
